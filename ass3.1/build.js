...
function createHalo(radius, n, sx, sy, sz) {
    var group = new THREE.Group(); // create empty group to hold halo panels
    var material = new THREE.MeshBasicMaterial(); // basic material (no lighting)
    ...
    for (var i = 0; i < n; i++) { // loop to create n panels
        var scale = new THREE.Matrix4(); // scale cube into thin rectangle
        var rotation = new THREE.Matrix4(); // rotate panel to correct angle
        var translation = new THREE.Matrix4(); // translate panel to correct radius
        var combined = new THREE.Matrix4(); // combined transformation matrix
        var panel = new THREE.Mesh(geometry, material); // create mesh for panel

        scale.makeScale(sx, sy, sz); // scale cube into thin rectangle
        translation.makeTranslation(radius, 0, 0); // translate panel to correct radius from Sun
        rotation.makeRotationY(i * 2 * Math.PI / n);
        combined.multiply(rotation); // apply rotation
        combined.multiply(translation); // apply translation
        combined.multiply(scale); // apply scale
        
        var geometry = new THREE.BoxGeometry(1, 1, 1); // create unit cube geometry
        var panel = new THREE.Mesh(geometry, material); // create mesh for panel
        panel.matrixAutoUpdate = false; // disable automatic matrix updates
        panel.matrix.copy(combined); // apply combined transformation to panel
        group.add(panel); // add panel to halo group
    }
    return group; // return the completed halo group
}
...
function buildDyson() {
   ...
   var sx = 0.25; // panel width (arbitrary units)
   var sy = 1.2; // panel height (arbitrary units)
   var sz = 0.5; // panel thickness (arbitrary units)
   n = 30; // number of panels in each halo (arbitrary)

   haloA = createHalo(haloRadius, n, sx, sy, sz);
   haloB = createHalo(haloRadius+0.4, n, sx, sy, sz);
   haloC = createHalo(haloRadius+0.8, n, sx, sy, sz);
   haloD = createHalo(haloRadius+0.9, n, sx, sy, sz);
   haloB.rotation.x = Math.PI / 2; // rotate second halo to be perpendicular
   haloC.rotation.x = Math.PI / 4; // rotate third halo to be perpendicular
   haloD.rotation.x = (Math.PI*(Math.PI/2)) / 2; // rotate fourth halo to be perpendicular
